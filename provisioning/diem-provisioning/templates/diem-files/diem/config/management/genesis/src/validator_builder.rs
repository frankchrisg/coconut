...
impl ValidatorBuilder {
    pub fn new<T: AsRef<Path>>(config_directory: T, move_modules: Vec<Vec<u8>>) -> Self {
        Self {
            ...
            node_paths: String::new(),
            num_prepare_accounts: 0,
        }
    }
    ...
    pub fn node_args(mut self, node_args: String) -> Self {
        self.node_args = node_args;
        self
    }

        pub fn node_paths(mut self, node_paths: String) -> Self {
    ...
    pub fn num_prepare_accounts(mut self, num_prepare_accounts: u32) -> Self {
        self.num_prepare_accounts = num_prepare_accounts;
        self
    }
        ...
        R: ::rand::RngCore + ::rand::CryptoRng,
    {
        ...
        let split = self.node_args.split(",");
        let vec = split.collect::<Vec<&str>>();

        let split_path = self.node_paths.split(",");
        let vec_path = split_path.collect::<Vec<&str>>();
        
        // Generate and initialize Validator configs
        let mut validators = (0..self.num_validators.get())
            .map(|i| {
            
             let tpl_to_use = NodeConfig::default_tpl(Box::new(vec_path.get(i).unwrap()));
        ...
        R: ::rand::RngCore + ::rand::CryptoRng,
    {
        ...
        let mut validator =
            ValidatorConfig::new(name, storage_config, directory, tpl_to_use /*self.template.clone()*/);
        ...
        let vfn_network = NetworkConfig {
            listen_address: diem_config::utils::get_available_port_in_multiaddr_cust(vec),
    ...
    fn genesis_ceremony(
        ...
        let mut genesis_builder = GenesisBuilder::new(genesis_storage, num_prepare_accounts);
