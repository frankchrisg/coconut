...
num_prepare_accounts: u32
    ...
    {
        create_and_initialize_testnet_minting(&mut session, treasury_compliance_key, num_prepare_accounts);
    }
...
let addr_prefix = "11110";
let mut i = 0;

//thread::spawn(|| {

    let local: DateTime<Local> = Local::now();
    println!("Start account creation {:?}", local);

while i < num_prepare_accounts {

//    let mut test: Arc<Mutex<&mut Session<StateViewCache<GenesisStateView>>>> = Arc::new(Mutex::from(session));
...
let addr_with_suffix = addr_prefix.to_owned() + &i.to_string();
let addr_full = format!("{:1>32}", addr_with_suffix);
let addr_full_hex = "0x".to_string() + &addr_full;
let addr_account = AccountAddress::from_hex_literal(&addr_full_hex).expect("Parsing valid hex literal should always succeed");
let human_name = "moneyacc".to_owned() + &i.to_string();
let human_name_as_vec = human_name.as_bytes().to_vec();

    let create_dd_script_tmp = encode_create_designated_dealer_script_function(
        account_config::xus_tag(),
        0,
        addr_account,
        genesis_auth_key.prefix().to_vec(),
    ...
    exec_script_function(
        ...
        &create_dd_script_tmp,
    );


    let mint_max_xus_tmp = transaction_builder::encode_tiered_mint_script_function(
        account_config::xus_tag(),
        0,
        addr_account,
        std::u64::MAX / 2,
    ...
    let treasury_compliance_account_address_tmp = account_config::treasury_compliance_account_address();
    exec_script_function(session, treasury_compliance_account_address_tmp, &mint_max_xus_tmp);
...
i = i + 1;
println!("{} <- number account address -> {}", i, addr_account);
  }
    let local: DateTime<Local> = Local::now();
    println!("End account creation {:?}", local);
