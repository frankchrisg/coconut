...
pub fn get_available_port_in_multiaddr(is_ipv4: bool) -> NetworkAddress {
    let ip_proto = if is_ipv4 {
        Protocol::Ip4("192.168.2.112".parse().unwrap())
    } else {
...
pub fn get_available_port_in_multiaddr_cust(vec: Option<&&str>) -> NetworkAddress {

    if vec.is_none()  {
        let ip_proto = Protocol::Ip4("0.0.0.0".parse().unwrap());
        return NetworkAddress::from(ip_proto).push(Protocol::Tcp(get_available_port()));
    }

    let split = vec.unwrap().split(":");
    ...
    let ip = *vec.get(0).unwrap();
    let port = *vec.get(1).unwrap();

    NetworkAddress::from(Protocol::Ip4(ip.parse().unwrap())).push(Protocol::Tcp(port.parse().unwrap()))
}
