...
import (
	...
	//"log"
	"os"
	memory "github.com/hyperledger/sawtooth-sdk-go/examples/memory-intensive_go/src/memory-intensive/handler"
	"syscall"
)
...
func main() {
	...
	/*f, err := os.OpenFile("memory-sawtooth.log",
	...
	prefix := memory.Hexdigest("memory")[:6]
	handler := memory.NewMemoryHandler(prefix)
