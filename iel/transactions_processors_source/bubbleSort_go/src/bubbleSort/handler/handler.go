...
func NewBubbleSortHandler(namespace string) *BubbleSortHandler {
...
const (
	familyName = "bubbleSort"
)

func (self *BubbleSortHandler) FamilyName() string {
	return familyName
}

func (self *BubbleSortHandler) FamilyVersions() []string {
	return []string{"0.1"}
}

func (self *BubbleSortHandler) Namespaces() []string {
	return []string{self.namespace}
}

func (self *BubbleSortHandler) BubbleSort(context *processor.Context, A []int, l int, r int, sig string) interface{} {
	for i := r; i >= l+1; i-- {
		for j := l; j <= i-1; j++ {
			if A[j] > A[j+1] {
				exchange(A, j, j+1)
	...
	var errEvent = context.AddEvent("sort/bubbleSort", attributes, data)
	if errEvent != nil {
		/*return &processor.InvalidTransactionError{
...
func (self *BubbleSortHandler) Apply(request *processor_pb2.TpProcessRequest, context *processor.Context) error {
	...
	if function == "Sort" {
		...
		var err = self.BubbleSort(context, A, l, r, signature)
		if err != nil {
...
func (self *BubbleSortHandler) preparePayload(request *processor_pb2.TpProcessRequest) (BubbleSortPayload, error) {

	var payloadData = request.GetPayload()
	if payloadData == nil {
		return BubbleSortPayload{}, &processor.InvalidTransactionError{Msg: "Must contain payload"}
	}
	var payload BubbleSortPayload
	var err = DecodeCBOR(payloadData, &payload)
	if err != nil {
		return BubbleSortPayload{}, &processor.InvalidTransactionError{
