...
type MergeSortRecPayload struct {
	Function string
	Args     []string
}
...
func NewMergeSortRecHandler(namespace string) *MergeSortRecHandler {
	return &MergeSortRecHandler{
		namespace: namespace,
	}
}

const (
	familyName = "mergeSortRec"
)

func (self *MergeSortRecHandler) FamilyName() string {
	return familyName
}

func (self *MergeSortRecHandler) FamilyVersions() []string {
	return []string{"0.1"}
}

func (self *MergeSortRecHandler) Namespaces() []string {
	return []string{self.namespace}
}

func (self *MergeSortRecHandler) MergeSortRec(context *processor.Context, A []int, l int, r int) interface{} {
	if l < r {
		var q = (l + r) / 2
		self.MergeSortRec(context, A, l, q)
		self.MergeSortRec(context, A, q+1, r)
		merge(A, l, q, r)
	}
	return nil
}

func (self *MergeSortRecHandler) Apply(request *processor_pb2.TpProcessRequest, context *processor.Context) error {
	...
	if function == "Sort" {
		...
		var err = self.MergeSortRec(context, A, l, r)
		if err != nil {
		...
		var errEvent = context.AddEvent("sort/mergeSortRec", attributes, data)
		if errEvent != nil {
			/*return &processor.InvalidTransactionError{
...
func (self *MergeSortRecHandler) preparePayload(request *processor_pb2.TpProcessRequest) (MergeSortRecPayload, error) {

	var payloadData = request.GetPayload()
	if payloadData == nil {
		return MergeSortRecPayload{}, &processor.InvalidTransactionError{Msg: "Must contain payload"}
	}
	var payload MergeSortRecPayload
	var err = DecodeCBOR(payloadData, &payload)
	if err != nil {
		return MergeSortRecPayload{}, &processor.InvalidTransactionError{
