...
type IOExtendedPayload struct {
	Function string
	Args     []string
}

type IOExtendedHandler struct {
	namespace string
}

func NewIOExtendedHandler(namespace string) *IOExtendedHandler {
	return &IOExtendedHandler{
		namespace: namespace,
	}
}

const (
	familyName = "io_extended"
)

func (self *IOExtendedHandler) FamilyName() string {
	return familyName
}

func (self *IOExtendedHandler) FamilyVersions() []string {
	return []string{"0.1"}
}

func (self *IOExtendedHandler) Namespaces() []string {
	return []string{self.namespace}
}

func (self *IOExtendedHandler) Write(context *processor.Context, args []string) interface{} {
	...
	for i := 0; i < size; i++ {
		...
		var exists, errGet = context.GetState([]string{sKAddress})
		if errGet != nil {
			/*return &processor.InvalidTransactionError{
				Msg: fmt.Sprint("Error getting the state"),
			}*/
			logger.Error("Error getting the state")
			return nil
		}

		_, ok := exists[sKAddress]
		if ok {
			...
			var errEvent = context.AddEvent("ioExtended/write/exist", attributes, data)
			if errEvent != nil {
				/*return &processor.InvalidTransactionError{
...
func (self *IOExtendedHandler) Scan(context *processor.Context, args []string) interface{} {
	...
	for i := 0; i < size; i++ {
		...
		if !ok {
			...
			var attributes = make([]processor.Attribute, 0)
			var errEvent = context.AddEvent("ioExtended/scan/doesntexist", attributes, data)
			if errEvent != nil {
				/*return &processor.InvalidTransactionError{
					Msg: fmt.Sprint("Failed to add event: ", errEvent),
				}*/
...
func (self *IOExtendedHandler) RevertScan(context *processor.Context, args []string) interface{} {
	...
	for i := 0; i < size; i++ {
		...
		if !ok {
			...
			var attributes = make([]processor.Attribute, 0)
			var errEvent = context.AddEvent("ioExtended/revertScan/doesntexist", attributes, data)
			if errEvent != nil {
				/*return &processor.InvalidTransactionError{
					Msg: fmt.Sprint("Failed to add event: ", errEvent),
				}*/
...
func (self *IOExtendedHandler) Apply(request *processor_pb2.TpProcessRequest, context *processor.Context) error {
...
func (self *IOExtendedHandler) preparePayload(request *processor_pb2.TpProcessRequest) (IOExtendedPayload, error) {

	var payloadData = request.GetPayload()
	if payloadData == nil {
		return IOExtendedPayload{}, &processor.InvalidTransactionError{Msg: "Must contain payload"}
	}
	var payload IOExtendedPayload
	var err = DecodeCBOR(payloadData, &payload)
	if err != nil {
		return IOExtendedPayload{}, &processor.InvalidTransactionError{
...
func DecodeCBOR(data []byte, pointer interface{}) error {
	defer func() error {
		if recover() != nil {
			...
			logger.Errorf("Failed to decode payload")
			return nil
		}
		return nil
	}()
