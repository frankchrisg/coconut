...
tpsAndLatencyDataRep <- tpsAndLatencyDataRep[tpsAndLatencyDataRep$run_id != "4000000-quorumConfiguration-sb-sendPayment-40-flalv-rn-1-hetzner-repid-0-quorum-rl-400-ib-5" | (tpsAndLatencyDataRep$run_id == "4000000-quorumConfiguration-sb-sendPayment-40-flalv-rn-1-hetzner-repid-0-quorum-rl-400-ib-5" & tpsAndLatencyDataRep$stf == "2022-07-20 09:50:43+02"),]
tpsAndLatencyDataRep <- tpsAndLatencyDataRep[tpsAndLatencyDataRep$run_id != "4000000-quorumConfiguration-sb-balance-40-flalv-rn-1-hetzner-repid-0-quorum-rl-400-ib-5" | (tpsAndLatencyDataRep$run_id == "4000000-quorumConfiguration-sb-balance-40-flalv-rn-1-hetzner-repid-0-quorum-rl-400-ib-5" & tpsAndLatencyDataRep$stf == "2022-07-20 10:01:56+02"),]
...
for (benchmark in benchmarkList) {
  ...
  preparedDataFrameLatency <- subset(preparedDataFrameLatency, RL != "RL=200" & RL != "RL=800" & RL != "RL=1600")
  ...
  save(paste("quorum-plot-all-data-rows-", tempName, sep = ""),
       plotAllDataRows, "sentfailed")
  
  # plotTps -------------------------------------------------------------------
  
  plotTps = ggplot(data = longFormatTps, aes(x = RL, y = value)) + #, shape = BenchmarkName)) + #,color = BP.1
  ...
  save(paste("quorum-plot-tps-", tempName, sep = ""), plotTps, "parts")
  ...
  plotLatency = ggplot(data = longFormatLatency, aes(x = RL, y = value)) + #, shape = BenchmarkName)) + #,color = BP.1
  ...
  save(paste("quorum-plot-latency-", tempName, sep = ""),
       plotLatency, "parts")
  
  save(
    paste("quorum-plot-tpsandlatency-", tempName, sep = ""),
...
getByHighlight <-
  getByHighlight[c("BenchmarkName", "RL", "BP", "Max TPS")]

getByHighlight$RL = str_replace(getByHighlight$RL, "RL=", "")
getByHighlight$BP = str_replace(getByHighlight$BP, "BP=", "")
getByHighlight$BP = str_replace(getByHighlight$BP, "s", "")
...
for (i in 1:length(datalistLatencyTemp)) {
  ...
  datalistLatencyTemp[[i]]$Category <- gsub("BP=", "", datalistLatencyTemp[[i]]$Category)
  ...
  for(j in 1:nrow(datalistLatencyTemp[[i]])) {
    for(k in 1:nrow(getByHighlight)) {
      if(as.numeric(as.character(datalistLatencyTemp[[i]]$RL[j])) == getByHighlight$RL[k]
         & as.numeric(as.character(datalistLatencyTemp[[i]]$Category[j])) == getByHighlight$BP[k]
...
for(duration in bmsDuration) {
  ...
  if(length(summariseByMeanTpsAndLatency[summariseByMeanTpsAndLatency$fullBenchmarkName==duration &
                                         ...
                                         getByHighlight[cntDuration, ]$BP == summariseByMeanTpsAndLatency$cutCriteria
                                         ,]$duration) == 0 ||
...
colnames(summariseByMaxFailedAndSuccessful)[1] <- "Benchmark"
colnames(summariseByMaxFailedAndSuccessful)[2] <- "BP"
...
summariseByMaxFailedAndSuccessful <-
  summariseByMaxFailedAndSuccessful[c("Benchmark", "RL", "BP", "Successful", "Failed")]
...
print(
  xtable(xt, align = "l|ccccc", caption = "Quorum"),
  #"Sawtooth"),
  include.rownames = FALSE,
  sanitize.text.function = identity,
  type = 'latex'
