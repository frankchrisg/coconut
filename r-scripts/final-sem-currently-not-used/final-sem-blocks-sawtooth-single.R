...
basicSystem <-
  read.table('C:/Users/parallels/Downloads/sawtooth.txt',
             sep = '&',
             comment.char = '\\')
basicSystem$bs <- "Hyperledger Sawtooth"
...
save <- function(titleVar, plot) {
  fileName = tolower(gsub(
    " ",
    "",
    paste("C:/Users/parallels/Downloads/final-plots/sem-blocks-sawtooth-single/", titleVar, ".png"),
    fixed = TRUE
  ))
  ggsave(
    fileName,
...
for (valueToUse in valList) {
  ...
  allTpsAndBlockData$cutCriteria <-
    str_split_fixed(allTpsAndBlockData$run_id, "-", Inf)[, 24]
