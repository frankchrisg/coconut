...
basicSystem <-
  read.table('C:/Users/parallels/Downloads/diemopt.txt',
             sep = '&',
             comment.char = '\\')
basicSystem$bs <- "Diem"
...
save <- function(titleVar, plot) {
  fileName = tolower(gsub(
    " ",
    "",
    paste("C:/Users/parallels/Downloads/final-plots/sem-blocks-diem-single/", titleVar, ".png"),
    fixed = TRUE
  ))
  ggsave(
    fileName,
...
for (valueToUse in valList) {
  ...
  yAxisVal <- ""
  if(valueToUse=="cnt") {
    yAxisVal <- "Mean Blockcount"
  } else if (valueToUse=="sumnof"){
    yAxisVal <- "Mean sum of transactions"
  } else if(valueToUse=="sumnoa") {
    yAxisVal <- "Mean sum of actions"
  }
  
  fNameFullAllSuccessful <-
    "C:/Users/parallels/Downloads/allblockdataonly-diemonly"
  
  fNameFullAllFailed <-
    "C:/Users/parallels/Downloads/allfailedx3-diemonly"
  ...
  allTpsAndBlockData$sumnof[allTpsAndBlockData$sumnof > 0] <- sapply(allTpsAndBlockData$sumnof[allTpsAndBlockData$sumnof > 0], `-`, 1)
  allTpsAndBlockData$sumnoa[allTpsAndBlockData$sumnoa > 0] <- sapply(allTpsAndBlockData$sumnoa[allTpsAndBlockData$sumnoa > 0], `-`, 1)
  ...
  allTpsAndBlockData$sumnof <-
    replace(
      allTpsAndBlockData$sumnof,
      as.numeric(allTpsAndBlockData$sumnof) == 0,
      NA
    )
  
  allTpsAndBlockData$sumnoa <-
    replace(
      allTpsAndBlockData$sumnoa,
      as.numeric(allTpsAndBlockData$sumnoa) == 0,
      NA
    )
  
  allTpsAndBlockData$cnt <-
    replace(
      allTpsAndBlockData$cnt,
      as.numeric(allTpsAndBlockData$cnt) == 0,
      NA
    )
  ...
  generalGroup <-
    allTpsAndBlockData %>% group_by(fullBenchmarkName, cutCriteria, rl, received_time)
  
  summariseByReceivedTimeFirst <- generalGroup %>% summarise(
    # "cnt","sumnof","sumnoa","minbn","maxbn","received_time","cid","run_id","bs","bm","bmf"
    cnt = mean(cnt, na.rm = TRUE),
    sumnof = mean(sumnof, na.rm = TRUE),
    sumnoa = mean(sumnoa, na.rm = TRUE),
    n = n(),
    .groups = "keep"
  )
  
  #summariseByReceivedTimeFirst instead of allTpsAndBlockData
  if(valueToUse != "cnt") {
  ...
  preparedDataFrameSummary <- data.frame(
    ...
    #"CountAll" = allTpsAndBlockData$sumnof,
  ...
  for (benchmark in benchmarkList) {
    ...
    cutCriteriaParam <-
      as.numeric(getByHighlight$cutCriteria[(getByHighlight$BenchmarkName == benchmark)])
    
    plotSem <-
      ggplot(data = summariseByMeanAndSd[which(
        summariseByMeanAndSd$fullBenchmarkName == benchmark &
          summariseByMeanAndSd$cutCriteria == getByHighlight[getByHighlight$BenchmarkName == benchmark, ]$cutCriteria &
        ...
        x=benchmark,
        y=summariseByMeanAndSd$mean[which(
          summariseByMeanAndSd$fullBenchmarkName == benchmark &
            summariseByMeanAndSd$cutCriteria == getByHighlight[getByHighlight$BenchmarkName == benchmark, ]$cutCriteria &
            summariseByMeanAndSd$rl == getByHighlight[getByHighlight$BenchmarkName == benchmark, ]$RL
        )],
      ...
      stat_summary(
        ...
        fun.args = list(summariseByMeanAndSd$fullBenchmarkName[which(
          summariseByMeanAndSd$fullBenchmarkName == benchmark &
            summariseByMeanAndSd$cutCriteria == getByHighlight[getByHighlight$BenchmarkName == benchmark, ]$cutCriteria &
            summariseByMeanAndSd$rl == getByHighlight[getByHighlight$BenchmarkName == benchmark, ]$RL
        )]
      ...
      scale_x_discrete(labels = c(
        paste(
          ...
          " BS=",
          getByHighlight[getByHighlight$BenchmarkName == benchmark, ]$cutCriteria,
          " RL=",
