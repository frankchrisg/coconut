...
basicSystem <-
  read.table('C:/Users/parallels/Downloads/graphene.txt',
             sep = '&',
             comment.char = '\\')
basicSystem$bs <- "Graphene"
...
prepareFiles <- function(fileName) {
  ...
  fileToUseFullAll <-
    filter(fileToUseFullAll,
           grepl(".*-donothing-doNothing-.*-opt-100-.*|.*-keyvalue-set-.*-opt-50-.*|.*-keyvalue-get-.*-opt-50-.*|.*-sb-createAccount-.*-opt-100-.*|.*-sb-sendPayment-.*-opt-100-.*|.*-sb-balance-.*-opt-100-.*", run_id, ignore.case = FALSE))
...
save <- function(titleVar, plot) {
  fileName = tolower(gsub(
    " ",
    "",
    paste("C:/Users/parallels/Downloads/final-plots/sem-blocks-graphene-single/", titleVar, ".png"),
    fixed = TRUE
  ))
  ggsave(
    fileName,
