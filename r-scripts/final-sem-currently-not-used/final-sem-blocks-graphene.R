...
# replace 24]
#adjust by arrange 400 etc
#check x y axis check description
# change bi mm/pd/bi etc maxmsg data.frame $category
# corda special handling bi -arrange -category -(melt var) - facetwrap
#  / divisionByBlocks, teilen durch cnt bei mehr als 1 block pro sekunde
# groups keep summarise
# remove first mean
...
save <- function(titleVar, plot) {
  fileName = tolower(gsub(
    " ",
    "",
    paste("C:/Users/parallels/Downloads/final-plots/sem-blocks-graphene/", titleVar, ".png"),
    fixed = TRUE
  ))
  ggsave(
    fileName,
