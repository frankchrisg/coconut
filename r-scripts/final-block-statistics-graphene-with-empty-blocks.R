...
prepareFilesWithoutRepidReplacementCid <- function(fileName) {
  fNameFullAll <- fileName
  fileToUseFullAll <-
    read.csv(gsub(" ", "", paste(fNameFullAll)), dec = ".")
...
allBlocksRep <- prepareFilesRep(fNameFullAllRep)
allBlocksRep <- allBlocksRep[allBlocksRep$run_id != "4000000-grapheneConfiguration-sb-sendPayment-40-flalv-rn-1-hetzner-repid-0-graphene-rl-400-bi-10-opt-100-notpc-1",]
allBlocksRep <- allBlocksRep[allBlocksRep$run_id != "4000000-grapheneConfiguration-sb-createAccount-40-flalv-rn-1-hetzner-repid-0-graphene-rl-50-bi-5-opt-100-notpc-1",]
allBlocksRep <- allBlocksRep[allBlocksRep$run_id != "4000000-grapheneConfiguration-sb-sendPayment-40-flalv-rn-1-hetzner-repid-0-graphene-rl-400-bi-2-opt-100-notpc-1",]
allBlocksRep <- allBlocksRep[allBlocksRep$run_id != "4000000-grapheneConfiguration-sb-createAccount-40-flalv-rn-1-hetzner-repid-0-graphene-rl-400-bi-2-opt-100-notpc-1",]
allBlocksRep <- allBlocksRep[allBlocksRep$run_id != "4000000-grapheneConfiguration-sb-createAccount-40-flalv-rn-1-hetzner-repid-0-graphene-rl-400-bi-10-opt-100-notpc-1",]
allBlocksRep <- allBlocksRep[allBlocksRep$run_id != "4000000-grapheneConfiguration-sb-sendPayment-40-flalv-rn-1-hetzner-repid-0-graphene-rl-50-bi-5-opt-100-notpc-1",]
allBlocksRep$run_id<-gsub("hetzner","fixblockstatshetzner",allBlocksRep$run_id)
bs <- c()
bm <- c()
bmf <- c()
...
fNameFullAllRepCid <-
  "C:/Users/parallels/Downloads/failed7query-rt-cid-rid-data-1657982395344.csv" #fabric
allBlocksRepCid <- prepareFilesWithoutRepidReplacementCid(fNameFullAllRepCid)
allBlocksRepCid <- allBlocksRepCid[!(allBlocksRepCid$run_id %like% "4000000-grapheneConfiguration-sb-sendPayment-40-flalv-rn-1-hetzner-repid-.*-graphene-rl-400-bi-10-opt-100-notpc-1"),]
allBlocksRepCid <- allBlocksRepCid[!(allBlocksRepCid$run_id %like% "4000000-grapheneConfiguration-sb-createAccount-40-flalv-rn-1-hetzner-repid-.*-graphene-rl-50-bi-5-opt-100-notpc-1"),]
allBlocksRepCid <- allBlocksRepCid[!(allBlocksRepCid$run_id %like% "4000000-grapheneConfiguration-sb-sendPayment-40-flalv-rn-1-hetzner-repid-.*-graphene-rl-400-bi-2-opt-100-notpc-1"),]
allBlocksRepCid <- allBlocksRepCid[!(allBlocksRepCid$run_id %like% "4000000-grapheneConfiguration-sb-createAccount-40-flalv-rn-1-hetzner-repid-.*-graphene-rl-400-bi-2-opt-100-notpc-1"),]
allBlocksRepCid <- allBlocksRepCid[!(allBlocksRepCid$run_id %like% "4000000-grapheneConfiguration-sb-createAccount-40-flalv-rn-1-hetzner-repid-.*-graphene-rl-400-bi-10-opt-100-notpc-1"),]
allBlocksRepCid <- allBlocksRepCid[!(allBlocksRepCid$run_id %like% "4000000-grapheneConfiguration-sb-sendPayment-40-flalv-rn-1-hetzner-repid-.*-graphene-rl-50-bi-5-opt-100-notpc-1"),]
allBlocksRepCid$run_id<-gsub("hetzner","fixblockstatshetzner",allBlocksRepCid$run_id)
bs <- c()
bm <- c()
bmf <- c()
...
#select count(*), block_id, run_id
#from blockstatisticobject where
#run_id like '4000000-grapheneConfiguration-%-%-40-flalv-rn-1-hetzner-repid-%-graphene-rl-%-bi-%-opt-%-notpc-1'
#group by block_id, run_id
#having count(*) != 4

##blockGroup <- allBlocksCid %>% group_by(fullBenchmarkName, cutCriteria, rl, repid, block_id)

##summariseForkData <- blockGroup %>% summarise(
##  mblocks = count(block_id, na.rm = TRUE),
##  .groups = "keep"
...
for (benchmark in benchmarkList) {
  ...
  preparedDataFrame <- subset(preparedDataFrame, Category != "BI=2s" & Category != "BI=5s" & Category != "BI=10s")
  preparedDataFrame <- subset(preparedDataFrame, RL != "RL=200" & RL != "RL=400")
  ...
  blockPlot <-
    ...
    geom_point(position = position_dodge(width = .9), aes(color = variable), shape=c(1,1,2,2,5,5,6,6,7,7),  size=3) +#"\u25BA", size=2) +
    geom_text(
      aes(
    ...
    scale_y_continuous(limits = c(0
                                  , ceiling(max(longFormat$value,na.rm=TRUE) + (
                                    max(longFormat$value,na.rm=TRUE) / 10 * 4
                                  )))) +
    theme(
