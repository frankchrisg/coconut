...
fNameFullAll <-
  "C:/Users/parallels/Downloads/testwr-graphene"

allData <- prepareFiles(fNameFullAll)

allData$rl = str_split_fixed(allData$run_id, "-", Inf)[, 14]
allData$cutCriteria <-
  str_split_fixed(allData$run_id, "-", Inf)[, 16]
allData$opt <- str_split_fixed(allData$run_id, "-", Inf)[, 18]
##allData$notpc <- str_split_fixed(allData$run_id, "-", Inf)[, 20]
...
generalGroup <-
  allData %>%
  group_by(fullBenchmarkName, rl, opt, cutCriteria)
...
#summariseByMeanTpsAndLatency <- summariseByMeanTpsAndLatency %>% filter(opt == 1)
...
summariseByMeanTpsAndLatency$opt <-
  ifelse(
    summariseByMeanTpsAndLatency$highlight == "highlight",
    paste("\\textbf{", summariseByMeanTpsAndLatency$opt, "}", sep = ""),
    summariseByMeanTpsAndLatency$opt
  )
...
print(
  xtable(xtOptimal, align = "l|cccccc", caption = "Graphene"),
  include.rownames = FALSE,
  sanitize.text.function = identity
)
