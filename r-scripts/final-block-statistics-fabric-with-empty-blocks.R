...
allBlocksRep <- allBlocksRep[allBlocksRep$run_id != "4000000-fabricConfiguration-sb-createAccount-40-flalv-rn-1-hetzner-repid-0-fabric-rl-100-mm-2000",]
allBlocksRep <- allBlocksRep[allBlocksRep$run_id != "4000000-fabricConfiguration-sb-sendPayment-40-flalv-rn-1-hetzner-repid-0-fabric-rl-100-mm-2000",]
bs <- c()
bm <- c()
bmf <- c()
...
allBlocksRepCid <- allBlocksRepCid[!(allBlocksRepCid$run_id %like% "4000000-fabricConfiguration-sb-createAccount-40-flalv-rn-1-hetzner-repid-.*-fabric-rl-100-mm-2000"),]
allBlocksRepCid <- allBlocksRepCid[!(allBlocksRepCid$run_id %like% "4000000-fabricConfiguration-sb-sendPayment-40-flalv-rn-1-hetzner-repid-.*-fabric-rl-100-mm-2000"),]
bs <- c()
bm <- c()
bmf <- c()
...
for (benchmark in benchmarkList) {
  ...
  preparedDataFrame <-
    data.frame(
      ...
      "Category" = c(rep("MM=100", 4), rep("MM=500", 4), rep("MM=1000", 4), rep("MM=2000", 4))
    )
  
  ## remove from dataframe
  preparedDataFrame <- subset(preparedDataFrame, Category != "MM=500" & Category != "MM=1000" & Category != "MM=2000")
...
longFormatBak <- datalistSubsetBlocks[[5]] #longFormat
