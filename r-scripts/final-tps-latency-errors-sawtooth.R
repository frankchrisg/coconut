...
save <- function(titleVar, plot, path) {
  fileName = tolower(gsub(
    " ",
    "",
    paste("C:/Users/frank/Downloads/final-plots/sawtooth/",path,"/", titleVar, ".png"),
    fixed = TRUE
  ))
  ggsave(
    fileName,
...
fNameFullAll <-
  "C:/Users/frank/Downloads/testwr-cpbftbpd" #cpbftbpd"
...
#tpsAndLatencyDataRep$run_id<-gsub("flaoslv","flalv",tpsAndLatencyDataRep$run_id)
tpsAndLatencyDataRep <- tpsAndLatencyDataRep[tpsAndLatencyDataRep$run_id != "4000000-sawtoothConfiguration-keyvalue-set-40-flalv-rn-1-hetzner-repid-0-sawtooth-rl-400-notpbpc-100-nobpc-1-maxbtperbl-100000-notppc-100-cpbftbpd-2",]
tpsAndLatencyDataRep <- tpsAndLatencyDataRep[tpsAndLatencyDataRep$run_id != "4000000-sawtoothConfiguration-sb-sendPayment-40-flalv-rn-1-hetzner-repid-0-sawtooth-rl-200-notpbpc-100-nobpc-1-maxbtperbl-100000-notppc-100-cpbftbpd-1",]
tpsAndLatencyDataRep <- tpsAndLatencyDataRep[tpsAndLatencyDataRep$run_id != "4000000-sawtoothConfiguration-sb-sendPayment-40-flalv-rn-1-hetzner-repid-0-sawtooth-rl-200-notpbpc-100-nobpc-1-maxbtperbl-100000-notppc-100-cpbftbpd-10",]
tpsAndLatencyDataRep <- tpsAndLatencyDataRep[tpsAndLatencyDataRep$run_id != "4000000-sawtoothConfiguration-sb-createAccount-40-flalv-rn-1-hetzner-repid-0-sawtooth-rl-200-notpbpc-100-nobpc-1-maxbtperbl-100000-notppc-100-cpbftbpd-1",]
tpsAndLatencyDataRep <- tpsAndLatencyDataRep[tpsAndLatencyDataRep$run_id != "4000000-sawtoothConfiguration-sb-createAccount-40-flalv-rn-1-hetzner-repid-0-sawtooth-rl-200-notpbpc-100-nobpc-1-maxbtperbl-100000-notppc-100-cpbftbpd-10",]
...
tpsAndLatencyData$cutCriteria <-
  str_split_fixed(tpsAndLatencyData$run_id, "-", Inf)[, 24] # 24
...
for (benchmark in benchmarkList) {
  ...
  preparedDataFrameTps <-
    data.frame(
      ...
      "placeholder_name" = summariseByMeanTpsAndLatency$tps[which(summariseByMeanTpsAndLatency$fullBenchmarkName ==
                                                                    tempName)],
      "Category" = c(rep("PD=1s", 4), rep("PD=2s", 4), rep("PD=5s", 4), rep("PD=10s", 4)),
  ...
  preparedDataFrameTps <- subset(preparedDataFrameTps, Category != "PD=2s" & Category != "PD=5s")
  ...
  longFormatTps$Category <-
    factor(longFormatTps$Category,
           levels = c("PD=1s", "PD=2s", "PD=5s", "PD=10s"))
  ...
  preparedDataFrameLatency <- subset(preparedDataFrameLatency, Category != "PD=2s" & Category != "PD=5s")
  ...
  longFormatAllDataRows <- subset(longFormatAllDataRows, Category != "PD=2s" & Category != "PD=5s")
  ...
  save(paste("sawtooth-plot-all-data-rows-", tempName, sep = ""),
       plotAllDataRows, "sentfailed")
  
  # plotTps -------------------------------------------------------------------
  
  plotTps = ggplot(data = longFormatTps, aes(x = RL, y = value)) + #, shape = BenchmarkName)) + #,color = PD.1
  ...
  save(paste("sawtooth-plot-tps-", tempName, sep = ""), plotTps, "parts")
  ...
  plotLatency = ggplot(data = longFormatLatency, aes(x = RL, y = value)) + #, shape = BenchmarkName)) + #,color = PD.1
...
#               shape = c(15, 17, 18, NA)#(16, 16, 15, NA)
  ...
  save(paste("sawtooth-plot-latency-", tempName, sep = ""),
       plotLatency, "parts")
  
  save(
    paste("sawtooth-plot-tpsandlatency-", tempName, sep = ""),
...
colnames(getByHighlight)[2] <- "PD"
colnames(getByHighlight)[4] <- "Max TPS"

getByHighlight <-
  getByHighlight[c("BenchmarkName", "RL", "PD", "Max TPS")]

getByHighlight$RL = str_replace(getByHighlight$RL, "RL=", "")
getByHighlight$PD = str_replace(getByHighlight$PD, "PD=", "")
getByHighlight$PD = str_replace(getByHighlight$PD, "s", "")
...
for (i in 1:length(datalistLatencyTemp)) {
  ...
  datalistLatencyTemp[[i]]$Category <- gsub("PD=", "", datalistLatencyTemp[[i]]$Category)
  ...
  for(j in 1:nrow(datalistLatencyTemp[[i]])) {
    for(k in 1:nrow(getByHighlight)) {
      if(as.numeric(as.character(datalistLatencyTemp[[i]]$RL[j])) == getByHighlight$RL[k]
         & as.numeric(as.character(datalistLatencyTemp[[i]]$Category[j])) == getByHighlight$PD[k]
...
for(duration in bmsDuration) {
  ...
  if(length(summariseByMeanTpsAndLatency[summariseByMeanTpsAndLatency$fullBenchmarkName==duration &
                                         ...
                                         getByHighlight[cntDuration, ]$PD == summariseByMeanTpsAndLatency$cutCriteria
                                         ,]$duration) == 0 ||
...
print(
  xtable(xt, align = "l|cc|cccc", caption = "Hyperledger Sawtooth"),
  #"Sawtooth"),
...
colnames(summariseByMaxFailedAndSuccessful)[1] <- "Benchmark"
colnames(summariseByMaxFailedAndSuccessful)[2] <- "PD"
...
colnames(summariseByMaxFailedAndSuccessful)[6] <- "TXs"

summariseByMaxFailedAndSuccessful <-
  summariseByMaxFailedAndSuccessful[c("Benchmark", "RL", "PD", "Successful", "Failed", "TXs")]
...
longFormatLatencyBak <- datalistLatency[[4]] #longFormatLatency
longFormatLatencyBak$RL <- apply(longFormatLatencyBak, 1, function(x) gsub("RL=", " ", x[["RL"]], fixed = TRUE))
longFormatLatencyBak$SDLATENCY <- round(longFormatLatencyBak$SDLATENCY, digits = 2)
longFormatLatencyBak$value <- round(longFormatLatencyBak$value, digits = 2)
...
longFormatTpsBak <- datalistTps[[4]] #longFormatTps
longFormatTpsBak$RL <- apply(longFormatTpsBak, 1, function(x) gsub("RL=", " ", x[["RL"]], fixed = TRUE))
...
longFormatAllDataRowsBak <- datalistFailures[[4]] #longFormatAllDataRows
longFormatAllDataRowsBak$RL <- apply(longFormatAllDataRowsBak, 1, function(x) gsub("RL=", " ", x[["RL"]], fixed = TRUE))
longFormatAllDataRowsBak <- longFormatAllDataRowsBak[,-3]
