module 0x1::SmallBank {
    ...
    fun transact_savings_fun(account: &signer, amount: u64, sig: vector<u8>): () acquires SmallBank {
        ...
        collection.savings = collection.savings - amount;
    ...
    fun balance_fun(account: &signer, sender_as_str: vector<u8>, sig: vector<u8>)/*: u64*/ acquires SmallBank {
        let owner = Signer::address_of(account);
        let exist = exists<SmallBank>(owner);
        let collection = borrow_global_mut<SmallBank>(owner);
        ...
        let be: vector<u8> = b" __balance_event:";
        Vector::append(&mut prefix, be);
        let b: vector<u8> = b"-balance-";
        Vector::append(&mut prefix, b);
        let total_as_str = ConvertHelperUtils::convert_u64_to_vec_u8(total);
        Vector::append(&mut prefix, total_as_str);
        let aid: vector<u8> = b"-acctId-";
        Vector::append(&mut prefix, aid);
        Vector::append(&mut prefix, sender_as_str);
    ...
    public(script) fun balance(account: signer, sender_as_str: vector<u8>, sig: vector<u8>) acquires SmallBank {
        balance_fun(&account, sender_as_str, sig);
    }
