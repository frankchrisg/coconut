module 0x1::IO_ex {
    ...
    struct IO_ex has key, store, copy {
        sks: vector<u64>,
    ...
    fun write_fun(account: &signer, size: u64, start_key: u64, ret_len: u64, sig: vector<u8>) {
        ...
        let exist = exists<IO_ex>(owner);

        if (exist) {
            let handle = Event::new_event_handle<vector<u8>>(account);
            let prefix: vector<u8> = b"ioExtended/write/exist ";
        ...
        move_to(account, IO_ex{ sks: sks, states: states });
    ...
    fun scan_fun(account: &signer, size: u64, sig: vector<u8>): () acquires IO_ex {
        let owner = Signer::address_of(account);
        let exist = exists<IO_ex>(owner);
        let collection = borrow_global<IO_ex>(owner);

        if (!exist) {
            let handle = Event::new_event_handle<vector<u8>>(account);
            let prefix: vector<u8> = b"ioExtended/scan/doesntexist ";
            Vector::append(&mut prefix, sig);
            Event::emit_event(&mut handle, prefix);
            Event::destroy_handle(handle);
            return ()
    ...
    fun revert_scan_fun(account: &signer, sig: vector<u8>): () acquires IO_ex {
        let owner = Signer::address_of(account);
        let exist = exists<IO_ex>(owner);
        let collection = borrow_global<IO_ex>(owner);
        ...
        if (!exist) {
            let handle = Event::new_event_handle<vector<u8>>(account);
            let prefix: vector<u8> = b"ioExtended/revertScan/doesntexist ";
            Vector::append(&mut prefix, sig);
            Event::emit_event(&mut handle, prefix);
            Event::destroy_handle(handle);
            return ()
    ...
    public(script) fun write_ex(account: signer, size: u64, start_key: u64, ret_len: u64, sig: vector<u8>) {
        write_fun(&account, size, start_key, ret_len, sig);
    }

    public(script) fun scan_ex(account: signer, size: u64, sig: vector<u8>) acquires IO_ex {
        scan_fun(&account, size, sig);
    }

    public(script) fun revert_scan_ex(account: signer, sig: vector<u8>) acquires IO_ex {
