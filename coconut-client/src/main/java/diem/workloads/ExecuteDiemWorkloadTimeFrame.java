...
public class ExecuteDiemWorkloadTimeFrame implements IExecuteWorkload, IRequestDistribution {

    private static final Logger LOG = Logger.getLogger(ExecuteDiemWorkloadTimeFrame.class);
    ...
    private static final AtomicInteger ACCOUNT_COUNTER_GLOBAL = new AtomicInteger(0);
                ...
                try {
                    accountInformation = /*accountInformationMap.get("wl-" + workloadId).get(ACCOUNT_COUNTER.get(
                        "wl-" + workloadId + "-cl-" + clientObject.getClientId()).getAndIncrement());*/
                            AccountStore.getAccountInformationList(Configuration.FILE_NAME).get(ACCOUNT_COUNTER_GLOBAL.getAndIncrement());
                } catch (IndexOutOfBoundsException ex) {
                    LOG.error("Not enough accounts available...");
                    break;
                }
                startSequenceNumber = new AtomicLong(
                        Configuration.DISTRIBUTED_NONCE_HANDLING ?
