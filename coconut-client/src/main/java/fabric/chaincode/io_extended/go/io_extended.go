...
func main() {
	...
	fmt.Println("Started io_extended")
...
func (self *IOHandler) Write(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	for i := 0; i < size; i++ {
		...
		if state != nil {
			...
			var errEvent = stub.SetEvent("ioExtended/write/exist", []byte(signature))
			if errEvent != nil {
				return shim.Error("Error while setting event in Scan call")
			}
			return shim.Success(nil)
		}
...
func (self *IOHandler) Scan(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	for i := 0; i < size; i++ {
		...
		if state == nil {
			...
			var errEvent = stub.SetEvent("ioExtended/scan/doesntexist", []byte(signature))
			if errEvent != nil {
				return shim.Error("Error while setting event in Scan call")
			}
			return shim.Success(nil)
...
func (self *IOHandler) RevertScan(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	for i := 0; i < size; i++ {
		...
		if state == nil {
			var signature = args[2]
			var errEvent = stub.SetEvent("ioExtended/revertScan/doesntexist", []byte(signature))
			if errEvent != nil {
				return shim.Error("Error while setting event in RevertScan call")
			}
			return shim.Success(nil)
		}
