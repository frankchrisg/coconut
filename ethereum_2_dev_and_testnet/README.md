...
- the file pwd used with the parameter passphraseFile contains only the string test_password
...
To get some bootstrap files, run:
...
# the most needed files are execution_genesis.json, execution_genesis_block.json, config.toml, config.yaml, keymanager-token, network_key.json, deposit_contract_block.txt, deposit_contract_block_hash.txt
...
Boostrap both geth nodes as you would do with geth normally:
...
Add the peers to each other, replace 8\.192\.177\.168 with your IP:
...
Split the validators to multiple directories:
...
Save the output to the file (overwrite the file):
